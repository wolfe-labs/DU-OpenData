...
// Our regexes
const REGEX_CLASS = /---\s*@class (.*)/gi;
const REGEX_MULTICOMMENT = /---\s*(.*)/gi;
const REGEX_PARAM = /---\s*@param (.*)/gi;
const REGEX_DEPRECATED = /---\s*@deprecated (.*)/gi;
const REGEX_RETURN = /---\s*@return (.*)/gi;
const REGEX_PARENT = /self = (.*?)\(.*?\)/gi;
const REGEX_METHOD = /function self\.(.*?)\s*\(.*?\)/gi;
const REGEX_EVENT = /self\.(.*?)\s*=\s*Event:new()/gi;
...
module.exports = function LuaDoc (luaString) {
  ...
  source.forEach((line) => {
    ...
    // Detects general comments, this should always be last
